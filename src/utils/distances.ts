...
// Standard SI / imperial short forms — distinct from DistanceUnitShortLabel,
// which uses the running convention ("K" rather than "km").
export const DistanceUnitStandardShortLabel: Record<DistanceUnit, string> = {
...
export const getDistanceUnitLabel = (distanceUnit: DistanceUnit) =>
  DistanceUnitOptions.find(
    (distanceUnitOption) => distanceUnitOption.value === distanceUnit,
  )?.label;
...
// Canonical "<value><short-unit>" rendering (e.g. "7.5K", "500m", "10 mile").
export const formatDistanceWithShortUnit = (
  value: number,
  unit: DistanceUnit,
): string => `${formatDistanceValue(value)}${DistanceUnitShortLabel[unit]}`;
